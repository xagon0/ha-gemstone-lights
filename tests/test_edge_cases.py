...
async def test_edge_case_a_offline_restart(hass, entry, loaded_entry, vendor, monkeypatch):
...
async def test_edge_case_b_simultaneous_zone_edits(coordinator, vendor):
    ...
    tasks = [
        asyncio.create_task(coordinator.async_set_zone("hub", "front", {"color": 255, "brightness": 80, "animation": "Solid"})),
        asyncio.create_task(coordinator.async_set_zone("hub", "back", {"color": 65280, "brightness": 200, "animation": "chase"})),
    ]
    ...
    patterns = {entry["zoneId"]: entry["pattern"] for entry in vendor.writes[-1][3]["architectural"]["zonePatterns"]}
