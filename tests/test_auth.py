...
async def test_near_expiry_session_is_renewed_once_for_concurrent_requests(api, http):
    ...
    api._store({"access_token": jwt.encode({"exp": time.time() + 30}, "test-key" * 8), "refresh_token": "refresh"})
    ...
    with patch("pycognito.Cognito.renew_access_token", autospec=True, side_effect=renew) as aws:
    ...
    assert [call.kwargs["headers"]["authorization"] for calls in http.requests.values() for call in calls] == [f"Bearer {renewed}"] * 2
...
async def test_repeated_http_auth_rejection_stops_after_one_renewal(api, http):
    ...
    def renew(user):
        user.access_token = renewed
        user.id_token = renewed
    with patch("pycognito.Cognito.renew_access_token", autospec=True, side_effect=renew):
