...
@pytest.mark.parametrize("body", [[], {"state": None}, {"state": {"reported": {"currentlyPlaying": {}}}}, {"state": {"reported": {"currentlyPlaying": {"onState": True, "pattern": []}}}}])
async def test_malformed_local_state_uses_transport_error(hass, http, body):
    ...
    # When state is read, then it produces the fallback-compatible error, never off or AttributeError.
...
async def test_malformed_cloud_catalog_cannot_clear_discovery(api, http, body):
    ...
    # When discovery reads it, then it fails instead of reporting an empty account.
