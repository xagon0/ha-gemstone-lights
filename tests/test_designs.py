...
async def test_zone_pattern_design_uses_cloud_even_with_local_available(coordinator, vendor):
    ...
    design = {"id": "saved", "zonePatterns": [{"zoneId": "front", "pattern": {"animation": "chase", "colors": [255, 65280]}}]}
    ...
    assert vendor.writes == [("cloud", "hub", "/deviceControl/play/architectural", {"architectural": {**design, "preview": False}})]
...
async def test_static_pixel_design_uses_local_transport(coordinator, vendor):
    ...
    vendor.devices[0]["hub"] = {"localIp": "192.0.2.10", "tcpEnabled": True}
    coordinator.data = await coordinator._async_update_data()
    design = {"staticColors": [{"lights": [10, 11, 12], "color": 255}], "brightness": 80}
